    ...
    <div class="mx-auto text-white light:text-blueDark">
      <div v-if="step===1">
        ...
        <div class="flex items-center bg-primaryBg
                    ...
                    text-1rem lg:leading-2rem leading-1.6rem mb-1rem truncate">
          <span class="flex-1 truncate">{{ account }}</span>
          <i class="w-1.3rem h-1.3rem ml-1rem cursor-pointer icon-copy" @click="copyAddress(account)"></i>
        </div>
  ...
  methods: {
    ...
    async signup() {
      ...
      if (loginInfo) {
        ...
        // not authed
        this.showNotify(this.$t('signInView.notAuth'), 5000, 'error')
        this.step = 0
        this.$emit('back');
        this.authError = true
      }
